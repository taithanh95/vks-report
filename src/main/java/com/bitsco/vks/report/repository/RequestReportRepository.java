...
public interface RequestReportRepository extends JpaRepository<RequestReport, Long> {
    @Query(value = "SELECT a FROM RequestReport a WHERE 1 = 1 "
    ...
    List<RequestReport> findByCreatedByAndStatusInOrderById(String createdBy, List<Integer> status);

    RequestReport findFirstByGenId(String genId);
}
